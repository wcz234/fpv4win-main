...
void Aggregator::deinit_fec(void) {
    ...
    for (int ring_idx = 0; ring_idx < RX_RING_SIZE; ring_idx++) {
        ...
        rx_ring[ring_idx].fragment_map = nullptr;
        for (int i = 0; i < fec_n; i++) {
            delete[] rx_ring[ring_idx].fragments[i];
        }
        delete[] rx_ring[ring_idx].fragments;
        rx_ring[ring_idx].fragments = nullptr;
    }
...
void Aggregator::send_packet(int ring_idx, int fragment_idx) {
    ...
    uint64_t packet_seq = static_cast<uint64_t>(rx_ring[ring_idx].block_idx) * fec_k + fragment_idx;

    // 仅在单调前进时计数丢包，避免乱序/覆盖时把倒序重复计入
    if (packet_seq > static_cast<uint64_t>(seq) + 1 && seq > 0) {
        count_p_lost += static_cast<uint32_t>(packet_seq - seq - 1);
    }
    if (packet_seq > seq) {
        seq = static_cast<uint32_t>(packet_seq);
    }
