...
bool GifEncoder::encodeFrame(const shared_ptr<AVFrame> &frame) {
    ...
    AVFrame *frameToEncode = frame.get();
    ...
    if (_codecCtx->pix_fmt != frame->format) {
        ...
        if (!_tmpFrame) {
            ...
            _buff.resize(static_cast<size_t>(size));
            int ret = av_image_fill_arrays(
                _tmpFrame->data, _tmpFrame->linesize, _buff.data(), _codecCtx->pix_fmt,
                _codecCtx->width, _codecCtx->height, 1);   // FIX: 原来误传了 width 两次
            if (ret < 0) {
                return false;
            }
        }
        // 转换为GIF编码需要的颜色和高度
        int h = sws_scale(
            _imgConvertCtx, frame->data, frame->linesize, 0, frame->height,
            _tmpFrame->data, _tmpFrame->linesize);
        if (h != _codecCtx->height) {
            return false;
        }
        frameToEncode = _tmpFrame.get();
    }
    ...
    shared_ptr<AVPacket> pkt = shared_ptr<AVPacket>(av_packet_alloc(), [](AVPacket *p) { av_packet_free(&p); });
    if (!pkt) {
        return false;
    }
    ...
    _lastEncodeTime
        = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch())
              .count();
    // 发送帧到编码上下文
    int ret = avcodec_send_frame(_codecCtx.get(), frameToEncode);
    if (ret < 0) {
        return false;
    }
    // 获取已经编码完成的包（可能产生多个或 EAGAIN）
    bool wrote = false;
    while (true) {
