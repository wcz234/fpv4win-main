...
int detectQrCodes(
    ...
    points.release();
    if (results.size() < kMaxResults && detector.detectMulti(image, points) && !points.empty()) {
        decodedInfo.clear();
...
cv::Mat addWhiteBorder(const cv::Mat &source, int &borderX, int &borderY) {
    ...
    cv::Mat padded;
    cv::copyMakeBorder(source, padded, borderY, borderY, borderX, borderX, cv::BORDER_CONSTANT, cv::Scalar(255));
    return padded;
}
...
QVariantList QrCodeScanner::scan(const std::shared_ptr<AVFrame> &frame) {
    ...
    if (quickScale != 1.0) {
        detectQrCodes(
            impl.detector, impl.quickGray, frame->width, frame->height, results, 1.0 / quickScale, 1.0 / quickScale);
    }
    if (results.size() >= kMaxResults) {
        return results;
    }
    ...
    impl.closeGray = resizeToMaxSide(gray, kCloseMaxSide, closeScale);
    int borderX = 0;
    int borderY = 0;
    impl.borderedGray = addWhiteBorder(impl.closeGray, borderX, borderY);
    detectQrCodes(
        impl.detector, impl.borderedGray, frame->width, frame->height, results, 1.0 / closeScale, 1.0 / closeScale,
        borderX, borderY);
    if (results.size() >= kMaxResults) {
        return results;
    }
    ...
    detectQrCodes(impl.detector, impl.equalizedGray, frame->width, frame->height, results);
    if (results.size() >= kMaxResults) {
        return results;
    }
    ...
    detectQrCodes(impl.detector, impl.sharpenedGray, frame->width, frame->height, results);
    if (results.size() >= kMaxResults) {
        return results;
    }
    ...
    detectQrCodes(impl.detector, impl.upscaledGray, frame->width, frame->height, results, 1.0 / upscale, 1.0 / upscale);
    if (results.size() >= kMaxResults) {
        return results;
    }
    ...
    detectQrCodes(impl.detector, impl.binaryGray, frame->width, frame->height, results);
